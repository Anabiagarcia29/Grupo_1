...
int do_start_scheduling(message *m_ptr)
{
	...
	rmp->tickets = DEFAULT_LOTTERY_TICKETS;
	rmp->time_slice = DEFAULT_USER_TIME_SLICE;


	/* Take over scheduling the process. The kernel reply message populates
	 * the processes current priority and its time slice */
	if ((rv = sys_schedctl(0, rmp->endpoint, rmp->tickets, rmp->time_slice, 0))
!= OK)  {
...
int do_nice(message *m_ptr)
{
	...
	rmp = &schedproc[proc_nr_n];
	new_tickets = m_ptr->m_pm_sched_scheduling_set_nice.maxprio;
	if (new_tickets == 0) {
		new_tickets = 1;
	}
...
{
	int err;
	int new_tickets, new_quantum, new_cpu, niced;

	pick_cpu(rmp);
	new_tickets = rmp->tickets;
	...
	if ((err = sys_schedule(rmp->endpoint, new_tickets,
...
{
//Don't have more than one queues
}
